...
export default function Members() {
  ...
  // Split leadership into hierarchy groups
  const presidentsGroup = presidents.filter((m) => m.role === "Club President");
  const vpGroup = presidents.filter((m) => m.role === "Club Vice President");
  const hrGroup = presidents.filter((m) => m.role.toLowerCase().includes("human resources"));
  const pmGroup = presidents.filter((m) => m.role.toLowerCase().includes("project management"));

  // Helper function to render a leader card consistently
  const renderLeaderCard = (m: Member, idx: number, additionalClasses = "") => {
    return (
      <motion.div
        key={m.id}
        ...
        whileHover={{ y: -8 }}
        className={`${cardClass} ${additionalClasses}`}
        style={{ background: "linear-gradient(135deg, rgba(147,31,31,0.18), rgba(255,255,255,0.03))", backfaceVisibility: "hidden", WebkitBackfaceVisibility: "hidden", transform: "translate3d(0,0,0)" }}
      >
      ...
      <main className="relative z-[3] pt-[140px] px-10 pb-40 text-white overflow-hidden">
        <div className="max-w-[1400px] mx-auto">
          ...
          <div className="max-w-[1200px] mx-auto mb-28">
            {/* Presidency Label Row */}
            <div className="flex items-center gap-4 max-w-[800px] mx-auto mb-10 select-none">
              <div className="flex-1 h-px bg-white/10" />
              <span className="text-[10px] uppercase tracking-[0.3em] text-white/20 font-bold">Presidency</span>
              <div className="flex-1 h-px bg-white/10" />
            </div>

            {/* Level 1 & 2: Presidents & Vice President (Same Section) */}
            <div className="flex flex-col gap-6 max-w-[800px] mx-auto mb-20">
              {/* Presidents Grid */}
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-6 w-full items-start">
                {presidentsGroup.map((m, idx) => renderLeaderCard(m, idx))}
              </div>
              {/* Vice President Centered */}
              <div className="flex justify-center w-full items-start mt-2">
                {vpGroup.map((m, idx) => renderLeaderCard(m, idx, "w-full max-w-[380px]"))}
              </div>
            </div>
            ...
            <div className="flex items-center gap-4 max-w-[800px] mx-auto mb-10 select-none">
              <div className="flex-1 h-px bg-white/10" />
              <span className="text-[10px] uppercase tracking-[0.3em] text-white/20 font-bold">HR</span>
              <div className="flex-1 h-px bg-white/10" />
            </div>

            {/* Level 3A: Human Resources */}
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-6 max-w-[800px] mx-auto items-start mb-20">
              {hrGroup.map((m, idx) => renderLeaderCard(m, idx))}
            </div>

            {/* Project Management Label Row */}
            <div className="flex items-center gap-4 max-w-[800px] mx-auto mb-10 select-none">
              <div className="flex-1 h-px bg-white/10" />
              <span className="text-[10px] uppercase tracking-[0.3em] text-white/20 font-bold">Project Management</span>
              <div className="flex-1 h-px bg-white/10" />
            </div>

            {/* Level 3B: Project Management */}
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-6 max-w-[800px] mx-auto items-start">
              {pmGroup.map((m, idx) => renderLeaderCard(m, idx))}
            </div>
          </div>
          ...
          <motion.div initial={{ opacity: 0, y: 35 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.3, duration: 0.8, ease: [0.16, 1, 0.3, 1] }} className="text-center">
            ...
            <div className="flex flex-wrap justify-center gap-4 mb-14">
              {departments.map((dept) => (
                <button
                  ...
                  className={`px-6 py-3 rounded-full text-sm font-semibold tracking-wide border transition-all duration-300 ${activeDept === dept.id
            ...
            <AnimatePresence mode="wait">
              {activeDepartment && (
                ...
                >
                  {/* HEAD & VICE */}
                  <div className="flex flex-col sm:flex-row justify-center gap-6 mb-10 items-start">
